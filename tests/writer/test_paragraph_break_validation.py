...
class TestValidateParagraphBreaks:
    ...
    def test_validate_paragraph_breaks_sufficient(self):
        ...
        # Act
        is_valid, feedback = validate_paragraph_breaks(text_with_breaks, 1, "en")
    ...
    def test_validate_paragraph_breaks_wall_of_text(self):
        """Test that text with 0-1 breaks fails validation (Indonesian)."""
        # Arrange
        from Writer.Chapter.ParagraphValidator import validate_paragraph_breaks
        ...
        # Act
        is_valid, feedback = validate_paragraph_breaks(wall_of_text, 1, "id")
    ...
    def test_validate_paragraph_breaks_wall_of_text_english(self):
        """Test that text with 0-1 breaks fails validation (English)."""
        # Arrange
        from Writer.Chapter.ParagraphValidator import validate_paragraph_breaks
        ...
        # Act
        is_valid, feedback = validate_paragraph_breaks(wall_of_text, 1, "en")

        # Assert
        assert is_valid is False
        assert "too few paragraph breaks" in feedback
        assert "0" in feedback  # Current break count
        # Expected: max(3, 3000//500) = 6 breaks
        assert "6" in feedback or "minimum 6" in feedback
    ...
    def test_validate_paragraph_breaks_calculates_minimum(self):
        ...
        # Act
        is_valid, feedback = validate_paragraph_breaks(text_with_8_breaks, 1, "en")

        # Assert
        assert is_valid is False
        assert "8" in feedback  # Current count
        assert "10" in feedback or "minimum 10" in feedback  # Expected

    def test_validate_paragraph_breaks_returns_feedback(self):
        """Test that feedback message includes count and target (Indonesian)."""
        # Arrange
        from Writer.Chapter.ParagraphValidator import validate_paragraph_breaks
    ...
    def test_validate_paragraph_breaks_returns_feedback_english(self):
        """Test that feedback message includes count and target (English)."""
        # Arrange
        from Writer.Chapter.ParagraphValidator import validate_paragraph_breaks
        ...
        # Assert
        assert is_valid is False
        assert "0 paragraph" in feedback or "0" in feedback
        assert "2000 character" in feedback or "2000" in feedback
        assert "minimum 4" in feedback or "4 paragraph" in feedback
        assert "blank lines" in feedback

    def test_validate_paragraph_breaks_short_text(self):
        """Test that short text (<500 chars) still needs minimum 3 breaks (Indonesian)."""
        # Arrange
        from Writer.Chapter.ParagraphValidator import validate_paragraph_breaks
        ...
        # Act
        is_valid, feedback = validate_paragraph_breaks(short_text, 1, "id")

        # Assert
        assert is_valid is False
        assert "minimal 3" in feedback

    def test_validate_paragraph_breaks_short_text_english(self):
        """Test that short text (<500 chars) still needs minimum 3 breaks (English)."""
        # Arrange
        from Writer.Chapter.ParagraphValidator import validate_paragraph_breaks
        ...
        # Act
        is_valid, feedback = validate_paragraph_breaks(short_text, 1, "en")

        # Assert
        assert is_valid is False
        assert "minimum 3" in feedback
    ...
    def test_validate_paragraph_breaks_edge_cases(self):
        ...
        # Act
        is_valid, feedback = validate_paragraph_breaks(text_exactly_3, 1, "en")
