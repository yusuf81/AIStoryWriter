...
class TestSyntheticMaxTokensConfig:
    ...
    @patch.dict('os.environ', {'SYNTHETIC_API_KEY': 'test-key'})
    def test_synthetic_uses_industry_standard_temperature_for_structured(self):
        """Structured output should use temperature=0.0 (industry best practice)."""
        with patch('openai.OpenAI') as mock_openai_class:
            mock_client = MagicMock()
            mock_openai_class.return_value = mock_client
            ...
            # Verify temperature uses industry standard (0.0 for structured output)
            # Same as Ollama, Google, Grok, vLLM, Amazon Nova, Anyscale
            call_kwargs = mock_client.chat.completions.create.call_args[1]
