...
def main():
    ...
    if Args.Resume:
        ...
        try:
            ...
            if current_state.get('status') != 'in_progress':
                print(f"Run already completed or in unknown state ({current_state.get('status')}). Exiting.")
                sys.exit(1)
            ...
            for key, value in state_config.items():
                ...
                if hasattr(Writer.Config, key) and not callable(getattr(Writer.Config, key)) and not key.startswith('_'):
            ...
            if not log_directory or not os.path.isdir(log_directory):
                print(f"FATAL: Log directory '{log_directory}' not found or invalid in state file. Cannot resume.")
                sys.exit(1)
            SysLogger = Writer.PrintUtils.Logger(_ExistingLogDir=log_directory)
            SysLogger.Log(f"Successfully resumed run from state file: {state_filepath}", 5)

            # Pulihkan variabel penting dari state
            Prompt = current_state.get('translated_prompt_content') or current_state.get('input_prompt_content') # Prioritaskan terjemahan
            if not Prompt:
                 SysLogger.Log("FATAL: Could not find prompt content in state file.", 7)
                 sys.exit(1)
            BasePrompt = current_state.get('input_prompt_content') # Selalu simpan prompt asli
            ...
            Chapters = current_state.get('completed_chapters', []) # List bab yang sudah jadi
            ...
            Models = list(set([getattr(Writer.Config, model_var) for model_var in dir(Writer.Config) if model_var.endswith('_MODEL') and hasattr(Writer.Config, model_var)]))
        ...
        log_directory = SysLogger.LogDirPrefix # Dapatkan direktori log yang baru dibuat

        # Buat state awal
        current_state = {'status': 'in_progress', 'log_directory': log_directory, 'config': {}}
        ...
        current_state['state_filepath'] = state_filepath # Simpan path state itu sendiri
        ...
        if Args.Prompt is None:
            ...
            if 'input_prompt_file' in current_state['config'] and current_state['config']['input_prompt_file']:
                 Args.Prompt = current_state['config']['input_prompt_file']
                 SysLogger.Log(f"Warning: Using prompt file from config: {Args.Prompt}", 6)
            else:
                 SysLogger.Log("FATAL: No Prompt Provided (-Prompt) and none in config.", 7)
        ...
        if Writer.Config.TRANSLATE_PROMPT_LANGUAGE != "":
             SysLogger.Log(f"Translating prompt to {Writer.Config.TRANSLATE_PROMPT_LANGUAGE}...", 4)
             Prompt = Writer.Translator.TranslatePrompt(Interface, SysLogger, Prompt, Writer.Config.TRANSLATE_PROMPT_LANGUAGE)
             current_state['translated_prompt_content'] = Prompt # Simpan hasil terjemahan jika ada
             SysLogger.Log("Prompt translation complete.", 4)

        # Inisialisasi variabel lain untuk run baru
        Outline, Elements, RoughChapterOutline, BaseContext = None, None, None, None # Gunakan None untuk menandakan belum dibuat
        NumChapters = None
        ChapterOutlines = []
        Chapters = []
        start_chapter = 1
    ...
    if last_completed_step == 'init':
        ...
        Outline, Elements, RoughChapterOutline, BaseContext = (
            Writer.OutlineGenerator.GenerateOutline(
                Interface, SysLogger, Prompt, Writer.Config.OUTLINE_QUALITY # Gunakan Prompt (mungkin sudah diterjemahkan)
            )
        )
        ...
        SysLogger.Log("FATAL: Resuming after 'init' but outline data is missing in state.", 7)
        sys.exit(1)
    else:
    ...
    if last_completed_step == 'outline':
        ...
        last_completed_step = 'detect_chapters'
         ...
         SysLogger.Log("FATAL: Resuming after 'outline' but total_chapters data is missing in state.", 7)
         sys.exit(1)
    else:
         SysLogger.Log(f"Skipping Chapter Detection ({NumChapters} chapters already known).", 4)
    ...
    if Writer.Config.EXPAND_OUTLINE:
        if last_completed_step == 'detect_chapters':
            ...
            if not Outline or not NumChapters:
                 SysLogger.Log("FATAL: Cannot expand chapters, Outline or NumChapters is missing.", 7)
                 sys.exit(1)
            Messages = [Interface.BuildUserQuery(Writer.Prompts.EXPAND_OUTLINE_CHAPTER_BY_CHAPTER.format(_Outline=Outline))] # Reset messages untuk langkah ini
        ...
        elif last_completed_step not in ['expand_chapters', 'chapter_generation', 'chapter_generation_complete', 'post_processing', 'complete']:
             SysLogger.Log(f"FATAL: Resuming after '{last_completed_step}' but per-chapter outlines are missing.", 7)
             sys.exit(1)
        else:
            SysLogger.Log("Skipping Per-Chapter Outline Expansion (already completed or disabled).", 4)
    else:
...
"""
    ...
    if Writer.Config.EXPAND_OUTLINE and DetailedOutline: # Gunakan MegaOutline jika ekspansi aktif DAN berhasil
        UsedOutline = MegaOutline


    # Write the chapters (Mulai dari start_chapter)
    if last_completed_step in ['detect_chapters', 'expand_chapters', 'chapter_generation']:
        SysLogger.Log(f"Starting Chapter Writing from chapter {start_chapter}...", 5)
        # Pastikan Chapters adalah list (penting untuk resume)
        if not isinstance(Chapters, list): Chapters = []
        if NumChapters is None:
            SysLogger.Log("FATAL: Cannot start chapter generation, NumChapters is unknown.", 7)
            sys.exit(1)
        ...
        for i in range(start_chapter, NumChapters + 1):
            ...
            if Writer.Config.EXPAND_OUTLINE and ChapterOutlines and len(ChapterOutlines) >= i:
                ...
                SysLogger.Log(f"Using expanded outline for Chapter {i}", 6)
                 ...
                 SysLogger.Log(f"Warning: No specific outline found for Chapter {i}, using general outline.", 6)
                 CurrentChapterOutlineTarget = Outline # Fallback ke outline asli jika UsedOutline kosong

            if not CurrentChapterOutlineTarget:
                 SysLogger.Log(f"FATAL: No outline available for generating Chapter {i}.", 7)
                 sys.exit(1)
            ...
            if len(Chapters) >= i:
                 ...
                 SysLogger.Log(f"Overwriting existing Chapter {i} data during resume.", 6)
            else:
    ...
    elif last_completed_step not in ['chapter_generation_complete', 'post_processing', 'complete']:
         SysLogger.Log(f"FATAL: Resuming after '{last_completed_step}' but chapter generation was not completed.", 7)
         sys.exit(1)
    else:
    ...
    if last_completed_step == 'post_processing':
        ...
        StoryInfoJSON.update({"RoughChapterOutline": RoughChapterOutline if RoughChapterOutline else ""})
        StoryInfoJSON.update({"BaseContext": BaseContext if BaseContext else ""})
        StoryInfoJSON.update({"UnscrubbedChapters": Chapters}) # Simpan bab sebelum edit/scrub
        ...
        if Writer.Config.ENABLE_FINAL_EDIT_PASS:
             ...
             if not Chapters:
                 ...
                 NewChapters = Writer.NovelEditor.EditNovel(Interface, SysLogger, Chapters, Outline, NumChapters)
                 StoryInfoJSON.update({"EditedChapters": NewChapters}) # Simpan hasil edit
        ...
        if not Writer.Config.SCRUB_NO_SCRUB:
             ...
             if not NewChapters:
                  ...
                  NewChapters = Writer.Scrubber.ScrubNovel(Interface, SysLogger, NewChapters, NumChapters)
                  StoryInfoJSON.update({"ScrubbedChapter": NewChapters}) # Simpan hasil scrub
        ...
        if Writer.Config.TRANSLATE_LANGUAGE != "":
             SysLogger.Log(f"Starting Translation to {Writer.Config.TRANSLATE_LANGUAGE}...", 3)
             if not NewChapters:
                  SysLogger.Log("Warning: No chapters available for translation.", 6)
             else:
                  NewChapters = Writer.Translator.TranslateNovel(Interface, SysLogger, NewChapters, NumChapters, Writer.Config.TRANSLATE_LANGUAGE)
                  StoryInfoJSON.update({"TranslatedChapters": NewChapters}) # Simpan hasil terjemahan
        ...
        InfoQueryContent = StoryBodyText if StoryBodyText else (Outline if Outline else "No content available.")
        StoryInfoMessages = [Interface.BuildUserQuery(InfoQueryContent)]
        try:
            Info = Writer.StoryInfo.GetStoryInfo(Interface, SysLogger, StoryInfoMessages)
        ...
        gen_start_time_str = current_state.get('generation_start_time', datetime.datetime.now().strftime('%Y/%m/%d %H:%M:%S'))
        ...
        StatsString += f" - Enable Final Edit Pass: {Writer.Config.ENABLE_FINAL_EDIT_PASS}  \n"
        ...
        run_timestamp = datetime.datetime.strptime(gen_start_time_str, '%Y/%m/%d %H:%M:%S').strftime('%Y%m%d%H%M%S') if gen_start_time_str else datetime.datetime.now().strftime('%Y%m%d%H%M%S')
...
"""
        ...
        SysLogger.Log("Run completed successfully and state marked as completed.", 5)
         ...
         SysLogger.Log(f"Warning: Reached end of script with unexpected state '{last_completed_step}'.", 6)
