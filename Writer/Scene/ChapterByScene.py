...
def ChapterByScene(
    ...
    _Logger.Log(f"Starting Scene-By-Scene Generation Pipeline for Chapter {_ChapterNum}/{_TotalChapters}", 2)

    SceneBySceneOutline = Writer.Scene.ChapterOutlineToScenes.ChapterOutlineToScenes(
        Interface, _Logger, _ChapterNum, _TotalChapters, _ThisChapterOutline, _Outline, _BaseContext=_BaseContext # Pass chapter info
    )
    ...
    for idx, SceneOutline in enumerate(SceneJSONList): # Use enumerate for index
        ...
        RoughChapter += Writer.Scene.SceneOutlineToScene.SceneOutlineToScene(
            Interface, _Logger, SceneNum, TotalScenes, SceneOutline, _Outline, _BaseContext # Pass scene info
        )

    _Logger.Log(f"Finished Scene-By-Scene Generation Pipeline for Chapter {_ChapterNum}/{_TotalChapters}", 2)
